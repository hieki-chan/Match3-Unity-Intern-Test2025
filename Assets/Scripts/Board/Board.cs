...
public class Board
{
    ...
    public int BoardSize => BoardSizeX * BoardSizeY;
    ...
    //motion
    private float initSpeedMotion = .25f;

    public Board(Transform transform, int matchesMin, int BoardSizeX, int BoardSizeY, int order)
    {
        m_root = transform;

        m_matchMin = matchesMin;
    ...
    internal void Fill(List<(NormalItem.eNormalType, int)> itemList)
    {
        bool isTopBoard = itemList.Select(i => i.Item2).Sum() == BoardSize;

        Debug.Log(itemList.Select(i => i.Item2).Sum());
        ...
        for (int x = 0; x < BoardSizeX; x++)
        {
            for (int y = 0; y < BoardSizeY; y++)
            {
                ...
                var rand = UnityEngine.Random.Range(0, itemList.Count);
                var _item = itemList[rand];
                _item.Item2 -= 1;
                itemList[rand] = _item;

                item.SetType(itemList[rand].Item1);
                item.SetView();
                item.SetViewRoot(m_root);

                cell.Assign(item);
                cell.ApplyItemPosition(false);
                cell.SetActive(isTopBoard);
                ...
                cell.transform.DOScale(Vector3.one, initSpeedMotion).From(Vector3.zero);
                item.View.DOScale(Vector3.one, initSpeedMotion).From(Vector3.zero);
            }
        }
    }
